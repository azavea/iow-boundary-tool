...
#
locals {
  rds_database_identifier = lower("database${var.environment}")
}

resource "aws_db_subnet_group" "default" {
  # There's no advantage to use name_prefix or set a lifecycle rule here because
  # the resource is forever linked to the RDS instance.
  # https://github.com/hashicorp/terraform-provider-aws/issues/512
...
resource "aws_db_parameter_group" "default" {
  name_prefix = local.rds_database_identifier
...
module "database" {
  ...
  database_identifier        = local.rds_database_identifier
  ...
  final_snapshot_identifier  = lower("${var.environment}-rds-snapshot")
